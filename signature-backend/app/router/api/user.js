...
router.get("/getDelegatableOfficers/:officerId/:readerId", async (req, res) => {
  try {
    const { officerId, readerId } = req.params;
    const currentOfficer = await userModel.findOne({ id: officerId, status: status.active });
    if (!currentOfficer) {
      return res.status(404).json({ error: "Officer not found" });
    }
    const courtId = currentOfficer.courtId;
    const officers = await userModel.find(
      {
        courtId,
        status: status.active,
        $or: [
          { role: roles.officer, id: { $ne: officerId } },
          { role: roles.reader, id: readerId }
        ]
      },
      { id: 1, name: 1, role: 1, _id: 0 }
    );
    return res.json(officers);
  } catch (error) {
    console.log("error fetching delegatable officers", error);
    next(error);
  }
})
